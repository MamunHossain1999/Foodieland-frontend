...
const CategoryPage = () => {
  ...
  // রেসিপি ফিল্টারিং (activeCategory অনুযায়ী)
  const filteredRecipes = activeCategory
    ? recipes?.filter(recipe => recipe.category === activeCategory)
    : recipes;

  if (isLoading) return <p className="text-center mt-10">Loading...</p>;
  if (isError) return <p className="text-center mt-10 text-red-500">Error: {(error as any).message}</p>;
    ...
    incididunt ut labore et dolore magna aliquut enim ad minim.
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          ...
          {filteredRecipes?.length === 0 && (
            <p className="col-span-full text-center text-gray-600">No recipes found.</p>
          )}
        </div>
      </section>
    </div>
